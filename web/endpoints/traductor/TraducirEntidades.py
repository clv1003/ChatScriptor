...
def traducirEntidades(traductor, rootdir, chat):
    if os.path.exists(rootdir + chat + '/entities'):
        ...
        for entidad in entidades:
                ...
                for e in datos[1]:
                    value = e["value"]
                    diccionarioValue[value] = value
                    #synonyms = eval(str(e["synonyms"]))
                    # tr_synonyms = traductor.traducirFrase(synonyms)
                    # ProcesamientoEntidadesIntents.editar_s_ent(rootdir + chat, synonyms, entidad[0], tr_synonyms,
                    # traductor.getOriginal())
                tr_value = traductor.traducirDiccionario(diccionarioValue)

                for clave, valor in tr_value.items():
                    ProcesamientoEntidadesIntents.editar_v_ent(rootdir + chat, clave, entidad[0], valor,
                                                               traductor.getOriginal())
